...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    # 감지된 박스가 아예 없으면 패스
    if num_boxes[0] == 0:
        print("🎯 [스마트팜 진단] 감지된 잎사귀가 없습니다.")
        return image

    # 💡 1. 각 클래스별 감지 개수를 셀 바구니(투표함) 준비
    class_counts = {i: 0 for i in range(num_classes)}
    valid_indices = []

    # 신뢰도가 너무 낮은 노이즈 박스는 투표에서 제외 (최소 40% 이상만 인정)
    score_threshold = 0.4

    for i in range(num_boxes[0]):
        score = out_scores[0][i]
        class_ind = int(out_classes[0][i])
        if score >= score_threshold and 0 <= class_ind < num_classes:
            class_counts[class_ind] += 1
            valid_indices.append(i)

    # 투표 참여자가 없으면 기존 가장 높은 점수 1개만 강제 추출
    if len(valid_indices) == 0:
        best_idx = int(np.argmax(out_scores[0][:num_boxes[0]]))
        best_class_ind = int(out_classes[0][best_idx])
        valid_indices = [best_idx]
    else:
        # 💡 2. 다수결(가장 많이 카운트된 클래스 인덱스) 승자 선출
        best_class_ind = max(class_counts, key=class_counts.get)

    # 💡 3. 화면에는 '다수결로 승리한 클래스'의 박스 중 '가장 신뢰도가 높은 딱 1개'만 그리기
    best_box_idx = -1
    max_score = -1
    for i in valid_indices:
        if int(out_classes[0][i]) == best_class_ind:
            if out_scores[0][i] > max_score:
                max_score = out_scores[0][i]
                best_box_idx = i
    ...
    if best_box_idx != -1:
        ...
        fontScale = 0.5
        score = out_scores[0][i]
        bbox_color = colors[best_class_ind]
        ...
        if show_label:
            # 다수결 결과임을 명시
            bbox_mess = '%s: %.2f (Majority)' % (classes[best_class_ind], score)
    ...
    # 💡 4. 라즈베리 파이 제어기에서 읽을 수 있도록 터미널에 투표 현황과 종합 결론 정갈하게 출력
    print("\n📊 [잎사귀 개별 상태 투표 현황]")
    for c_idx, count in class_counts.items():
        print(f" ─ {classes[c_idx]}: {count}개 잎사귀에서 감지됨")
    print(f"🎯 종합 다수결 진단 최종 결과 -> 👑 {classes[best_class_ind].upper()} 👑\n")
