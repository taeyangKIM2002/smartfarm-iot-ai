...
class Dataset(object):
    ...
    def __next__(self):
        with tf.device("/cpu:0"):
            ...
            # 💡 YOLOv4-tiny(크기 2)인지 오리지널(크기 3)인지 동적 확인
            num_scales = len(self.train_output_sizes)
            ...
            # 💡 오리지널 YOLOv4일 때만 3번째 스케일(L) 할당 (IndexError 방지)
            if num_scales > 2:
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    # 💡 리턴 값을 튜플로 통째로 받아 언패킹 충돌 방지
                    true_boxes_outputs = self.preprocess_true_boxes(bboxes)

                    batch_image[num, :, :, :] = image
                    
                    # 💡 가중치 출력 창구 개수에 따른 동적 데이터 매핑
                    if len(true_boxes_outputs) == 6:
                        label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = true_boxes_outputs
                        ...
                        label_sbbox, label_mbbox, sbboxes, mbboxes = true_boxes_outputs
                ...
                # 💡 리턴할 때도 스케일 개수에 맞춰서 정확하게 패킹하여 리턴
                if num_scales > 2:
    ...
    def preprocess_true_boxes(self, bboxes):
        # 💡 현재 모델의 출력 창구 개수(tiny는 2, 오리지널은 3)를 동적으로 파악합니다.
        num_scales = len(self.train_output_sizes)
        ...
        bboxes_xywh = [np.zeros((self.max_bbox_per_scale, 4)) for _ in range(num_scales)]
        ...
        for bbox in bboxes:
            ...
            # 💡 NumPy 1.24 호환성을 위해 np.float 대신 np.float32 적용
        ...
        # 💡 개수에 맞춰 안전하게 동적 패킹하여 반환
        if num_scales > 2:
            return label[0], label[1], label[2], bboxes_xywh[0], bboxes_xywh[1], bboxes_xywh[2]
        else:
            return label[0], label[1], bboxes_xywh[0], bboxes_xywh[1]
